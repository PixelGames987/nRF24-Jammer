...
int channel_count = sizeof(ble_adv_priority) / sizeof(ble_adv_priority[0]);
...
void setup() {
  ...
  SPI.begin();
  SPI.setFrequency(10000000); // 10MHz - maximum reliable speed for nRF24L01+
  ...
  radio.setAddressWidth(3); // Minimum address width for speed
  radio.setPayloadSize(1); // Absolute minimum payload for max speed
...
void loop() {
  ...
  radio.setChannel(current_channels[current_channel_index]);
}
